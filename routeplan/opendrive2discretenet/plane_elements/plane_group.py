...
class ParametricLaneGroup:
    ...
    def __init__(
        ...
        inner_neighbour=None,    # 对车道Lane的相邻车道信息进行编码，返回相应的编码值，内侧（靠近参考线）车道编码、外侧车道编码、内侧车道是否同向(bool)
        inner_neighbour_same_direction=True,   # inner/outer_neighbour_id -> <road.id>.<lane_section.id>.<inner_lane.id/outer_lane.id>.<width.id>
    ...
    def to_discretelane(self, precision: float = 0.5) -> DiscreteLane:
        ...
        lanelet = DiscreteLane(
            self, left_vertices, center_vertices, right_vertices, self.id_, self.parametric_lanes[0].roadmark, self.parametric_lanes[0].objects,
            self.parametric_lanes[0].junction, self.parametric_lanes[0].left_lanes, self.parametric_lanes[0].right_lanes,
            self.parametric_lanes[0].center_roadmark
        )
