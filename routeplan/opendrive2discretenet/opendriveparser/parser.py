...
from .elements.roadObjects import Objects
...
def parse_opendrive_road_lane_section(newRoad, lane_section_id, lane_section):
    ...
    for sideTag, newSideLanes in sides.items():
        ...
        for lane in side.findall("lane"):
            ...
            # RoadMark
            '''对车道Lane的mark属性进行解析。车道的roadmark有可能为多组数据
            '''
            for Idx, roadmark in enumerate(lane.findall("roadMark")):
                # 遍历所有roadmark
                newRoadMark = RoadLaneSectionRoadMark()
                newRoadMark._soffset = float(roadmark.get("sOffset"))
                newRoadMark._type = roadmark.get("type")
                newRoadMark._width = float(roadmark.get("width"))
                newRoadMark._color = roadmark.get("color")
                newRoadMark._laneChange = roadmark.get("laneChange")
                # 对车道属性赋值
                new_lane._roadMark.append(newRoadMark)

            # lane_section的center_roadmark
            if new_lane.id == 0:
                newLaneSection._center_roadmark = new_lane._roadMark
...
def parse_opendrive_road(opendrive, road):
    ...
    newRoad.junction = int(road.get("junction"))

    junctionId = int(road.get("junction")) if road.get("junction") != "-1" else -1     # -1表示该road非交叉口内的connecting road
    ...
    # Objects
    road_objects = road.find("objects")     # 有空的 也有多个object
    if road_objects is not None:
        for Idx, object in enumerate(road_objects.findall("object")):
            # 遍历所有object
            objects = Objects()
            objects._id = float(object.get("id"))
            objects._type = object.get("type")
            objects._name = object.get("name")
            objects._s = float(object.get("s"))
            objects._t = float(object.get("t"))
            # 人行横道角点
            outline = object.find("outline")
            for i, cornerLocal_ in enumerate(outline.findall("cornerLocal")):
                cornerLocal = [float(cornerLocal_.get("u")), float(cornerLocal_.get("v")), float(cornerLocal_.get("z"))]
                objects.cornerLocal.append(cornerLocal)

            # 对车道属性赋值
            newRoad._objects.append(objects)
